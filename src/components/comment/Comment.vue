<template>
  <!--评论列表-->
  <div>
    <CommentForm v-if="parentCommentId === 0" />
    <h3 class="ui dividing header">Comments | 共 {{ totalComment }} 条评论</h3>
    <h3 class="ui header" v-if="totalComment === 0">快来抢沙发！</h3>
...
export default {
  ...
  computed: {
    ...mapState(['totalComment', 'comments', 'parentCommentId'])
  },
  methods: {
    setReply (id) {
